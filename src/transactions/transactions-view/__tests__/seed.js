...
/**
 * Seeds the underlying tables to populate the transactions_view
 * The view is a JOIN of: transaction_items, order_items, transactions, accounts, payment_types, transaction_types
 */
const seedOne = async (opts = {}) => {
    ...
    // Create a transaction_item - this populates the view
    const transactionItem = await DB.transactionItems.query(trx).insert({
        amount: 50.00 + testCounter,
    ...
    // Return the expected view record (what should appear in transactions_view)
    return {
        user_id: 1,
        account_id: account.id,
        amount: transactionItem.amount,
        entity_id: orderItem.entity_id,
        entity_type: orderItem.entity_type,
        transaction_type_id: transactionType.id,
        payment_type_id: paymentType.id,
        payment_type: paymentType.type,
        created_at: orderItem.created_at,
        transaction_type_label: transactionType.label,
        transaction_type_operation: transactionType.operation,
